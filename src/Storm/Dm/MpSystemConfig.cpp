...
MpSystemConfig::MpSystemConfig(Cpl::Dm::ModelDatabase &myModelBase,
                               const char *symbolicName)
    : ModelPointCommon_(myModelBase, symbolicName, &m_data, sizeof(m_data),
                        true) {
...
void MpSystemConfig::setToOff(
    Storm::Type::SystemConfig_T &cfgToReset) noexcept {
...
///////////////////////////////////////////////////////////////////////////////
void MpSystemConfig::attach(Observer &observer,
                            uint16_t initialSeqNumber) noexcept {
...
bool MpSystemConfig::isDataEqual_(const void *otherData) const noexcept {
  Storm::Type::SystemConfig_T *otherPtr =
      (Storm::Type::SystemConfig_T *)otherData;

  // Compare bounds array (must be done brute force since the array contains
  // floats)
  for (int i = 0; i < m_data.totalStages; i++) {
    if (!Cpl::Math::areFloatsEqual(m_data.stages[i].lowerBound,
                                   otherPtr->stages[i].lowerBound)) {
      return false;
    }
    if (!Cpl::Math::areFloatsEqual(m_data.stages[i].upperBound,
                                   otherPtr->stages[i].upperBound)) {
      return false;
    }
...
void MpSystemConfig::setJSONVal(JsonDocument &doc) noexcept {
  ...
  valObj["opMode"] =
      Storm::Type::OperatingMode::_from_integral_unchecked(m_data.currentOpMode)
          ._to_string();
  valObj["oduType"] =
      Storm::Type::OduType::_from_integral_unchecked(m_data.outdoorUnitType)
          ._to_string();
  valObj["iduType"] =
      Storm::Type::IduType::_from_integral_unchecked(m_data.indoorUnitType)
          ._to_string();
  ...
  for (int i = 0; i < m_data.totalStages; i++) {
    ...
    elemObj["cph"] =
        Storm::Type::Cph::_from_integral_unchecked(m_data.stages[i].cph)
            ._to_string();
...
bool MpSystemConfig::fromJSON_(JsonVariant &src, LockRequest_T lockRequest,
                               uint16_t &retSequenceNumber,
                               Cpl::Text::String *errorMsg) noexcept {
  Storm::Type::SystemConfig_T newVal = m_data;

  if (src.is<JsonObject>()) {
    // Numeric values
    if (src["numCompStages"].is<unsigned>()) {
      newVal.numCompressorStages =
          (uint16_t)src["numCompStages"].as<unsigned>();
    }
    if (src["numIdStages"].is<unsigned>()) {
      newVal.numIndoorStages = (uint16_t)src["numIdStages"].as<unsigned>();
    }
    if (src["totalStages"].is<unsigned>()) {
      newVal.totalStages = (uint16_t)src["totalStages"].as<unsigned>();
    }
    if (src["fanCont"].is<unsigned>()) {
      newVal.fanContinuousSpeed = (uint16_t)src["fanCont"].as<unsigned>();
    }
    ...
    if (enumVal) {
      ...
      auto maybeValue =
          Storm::Type::OperatingMode::_from_string_nothrow(enumVal);
      if (maybeValue) {
      ...
      for (unsigned i = 0; i < stageArray.size(); i++) {
        if (stageArray[i]["stage"].is<unsigned>()) {
          ...
          if (stageNum > 0 && stageNum <= newVal.totalStages) {
            newVal.stages[stageNum - 1].lowerBound =
                stageArray[i]["lower"] | newVal.stages[stageNum - 1].lowerBound;
            newVal.stages[stageNum - 1].upperBound =
                stageArray[i]["upper"] | newVal.stages[stageNum - 1].upperBound;
            newVal.stages[stageNum - 1].minIndoorFan =
                stageArray[i]["minBlower"] |
                newVal.stages[stageNum - 1].minIndoorFan;
            newVal.stages[stageNum - 1].maxIndoorFan =
                stageArray[i]["maxBlower"] |
                newVal.stages[stageNum - 1].maxIndoorFan;
            newVal.stages[stageNum - 1].minOnTime =
                stageArray[i]["minOn"] | newVal.stages[stageNum - 1].minOnTime;
            newVal.stages[stageNum - 1].minOffTime =
                stageArray[i]["minOff"] |
                newVal.stages[stageNum - 1].minOffTime;
