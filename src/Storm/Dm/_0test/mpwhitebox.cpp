...
static MpWhiteBox          mp_apple_( modelDb_, "APPLE" );
static MpWhiteBox          mp_orange_( modelDb_, "ORANGE" );
...
TEST_CASE( "MpWhiteBox" )
{
    ...
    Storm::Type::WhiteBox_T expectedVal;
    ...
    SECTION( "read/writes" )
    {
        expectedVal ={ false,true };
        mp_apple_.write( expectedVal );
        valid = mp_apple_.read( value );
        REQUIRE( valid );
        REQUIRE( memcmp( &value, &expectedVal, sizeof( value ) ) == 0 );

        expectedVal.abortOnOffCycle = false;
        mp_apple_.resetPulseSettings();
        valid = mp_apple_.read( value );
        REQUIRE( valid );
        REQUIRE( memcmp( &value, &expectedVal, sizeof( value ) ) == 0 );
    }
    ...
    SECTION( "toJSON-pretty" )
    {
        ...
        REQUIRE( val["abortOnOffCycle"] == false );
    }

    SECTION( "fromJSON" )
    {
        expectedVal ={ false, true };
        const char* json = "{\"name\":\"APPLE\",\"val\":{defeatEquipMinOffTime:false,abortOnOffCycle:true}}";
    ...
    SECTION( "observer" )
    {
        ...
        expectedVal ={ true, false };
        Viewer<MpWhiteBox, Storm::Type::WhiteBox_T>    viewer_apple1( t1Mbox, Cpl::System::Thread::getCurrent(), mp_apple_, expectedVal );
