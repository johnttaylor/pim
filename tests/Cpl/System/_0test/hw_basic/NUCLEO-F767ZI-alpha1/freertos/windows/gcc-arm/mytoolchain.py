...
bsp_mx        = os.path.join( "src", "Bsp", "ST", "NUCLEO-F767ZI", "alpha1", "MX" )
sdk_root      = os.path.join( NQBP_PKG_ROOT(), "xsrc", "stm32F7-SDK")
bsp_mx_root   = os.path.join( NQBP_PKG_ROOT(), bsp_mx )
freertos_root = os.path.join( NQBP_PKG_ROOT(), "xsrc", "FreeRTOS")
sysview_root  = os.path.join( NQBP_PKG_ROOT(), "src", "Bsp", "ST", "NUCLEO-F767ZI", "alpha1", "SeggerSysView" )
...
target_flags             = '-DUSE_STM32F7XX_NUCLEO_144 -DSTM32F767xx'
...
def create():
    lscript  = 'STM32F767ZITx_FLASH.ld'
